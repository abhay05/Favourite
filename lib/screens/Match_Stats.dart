...
class _MatchStatsState extends State<MatchStats> {
  ...
  Future<void> _loadStats(BuildContext ctx,String team1,String team2) async {
    await Provider.of<MatchStatsProvider>(ctx, listen: false).fetchStats(team1,team2,widget.startTime);
  }
    ...
    return Expanded(
      child: Row(
        children: [
          ...
          Expanded(child: Center(child: Text(stat_attributes_names[attribute]))),
  ...
  void initState(){
   ...
   print("Match Stats "+widget.team1.toString()+" "+widget.team2.toString());
    _statsFuture=Provider.of<MatchStatsProvider>(context,listen: false).fetchStats(widget.team1, widget.team2,widget.startTime);
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
   ...
   // var teams=ModalRoute.of(context).settings.arguments as Map<dynamic,dynamic>;
    ...
    return Scaffold(
        body: FutureBuilder(future: _statsFuture,builder: (ctx,dataSnapshot){
          Widget widget;
          if(dataSnapshot.connectionState==ConnectionState.waiting){
            ...
            if(dataSnapshot.error!=null){
             ...
             widget= Consumer<MatchStatsProvider>(builder: (ctx,provider,child){
               matchStats=provider.getStats();
             //  var statRows=getStats(matchStats.home_team_stats, matchStats.away_team_stats);
               widget=Container(
                 margin: EdgeInsets.only(top:MediaQuery.of(context).padding.top+height*0.05),
      height: height,
      child: Column(
        children: [
          Container(
              ...
              child: Row(
                children: [
                  Expanded(
                    child: Center(
                        child: Column(
                          children: [Container(padding: EdgeInsets.all(10),child: Text(
                            matchStats.home_team_goals==null?"0":matchStats.home_team_goals.toString(),
                            style: TextStyle(
                              fontSize: 30,
                      ...
                      color:matchStats.home_team==matchStats.winner?Colors.green:Colors.white,
                      child: Text(
                            matchStats.home_team==null?"Def":matchStats.home_team,
                            style: TextStyle(
                              fontSize: 10,
                              //color: matchStats.home_team==matchStats.winner?Colors.green:Colors.black
                            ),
                        ...
                        children: [Container(
                         // color: Colors.pink,
                          padding: EdgeInsets.all(10),child: Text(
                            matchStats.away_team_goals==null?"0":matchStats.away_team_goals.toString(),
                            style: TextStyle(
                              fontSize: 30,
                              //color: matchStats.home_team==matchStats.winner?Colors.green:Colors.black
                            ),),),
                          Container(
                           // color:Colors.red,
                            padding: EdgeInsets.all(10),
                            color: matchStats.away_team==matchStats.winner?Colors.blue:Colors.white,
                            child: Text(
                              matchStats.away_team==null?"Def":matchStats.away_team,
                              style: TextStyle(
                                fontSize: 10,
          ...
          Container(
              ...
              child: Container(
               ...
               // color:Colors.grey[400],
                boxShadow: [BoxShadow(color: Colors.grey[400]),BoxShadow(color:Colors.grey[300],spreadRadius: -6.0,blurRadius: 10,),],),
                //color: Colors.grey[400],
                child: Column(
                  children: [
                    ...getStats(matchStats.home_team_stats, matchStats.away_team_stats),
